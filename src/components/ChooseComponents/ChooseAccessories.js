...
class ChooseAccessories extends Component {
                ...
                <div>
                    {this.props.state.components.map(item => (
                        <div key={item.id} className="component">
                            ...
                            <button value={item.id} onClick={(event) => this.addAccessories(event)}>Insert Component</button>
                        </div>
                    ))}
...
export default connect(mapStateToProps)(ChooseAccessories);